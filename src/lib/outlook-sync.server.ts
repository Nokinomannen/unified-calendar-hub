/**
 * Server-only core sync logic for Outlook (via connector gateway) and ICS.
 * Imported dynamically from outlook.functions.ts handlers — never from the
 * browser.
 */
...
export async function graphCall(secretEnv: string, path: string): Promise<Response> {
  ...
  return fetch(`${GATEWAY}/microsoft_outlook${path}`, {
    headers: {
...
export async function syncOutlookAccountCore(
  db: Db,
  userId: string,
  accountId: string,
  ...
  const account = acc as OutlookAccountRow;
  ...
  let path: string | null =
    `/me/calendarView?startDateTime=${encodeURIComponent(start)}&endDateTime=${encodeURIComponent(end)}&$top=200&$select=id,subject,isAllDay,isCancelled,start,end,location,bodyPreview`;
  while (path) {
  ...
  const { data: existing } = await db
    ...
    .gte("start_at", start)
    .lte("start_at", end);
  const stale = ((existing ?? []) as { id: string; external_id: string }[]).filter(
...
export async function syncIcsCalendarCore(
  db: Db,
  userId: string,
  calendarId: string,
): Promise<{ synced: number; removed: number }> {
