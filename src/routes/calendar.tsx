...
function CalendarPage() {
  ...
  const [view, setView] = useState<ViewMode>(() => (typeof window !== "undefined" && (localStorage.getItem("cal-view") as ViewMode)) || "month");
  useEffect(() => { try { localStorage.setItem("cal-view", view); } catch {} }, [view]);
  ...
  function openEdit(e: ExpandedEvent) {
    // Strip occurrence-only fields to get the master EventRow
      ...
      <div className="space-y-4">
        ...
        {view === "day" && (
          <div className="rounded-xl border border-border bg-card p-2">
            <button onClick={() => setDrawerDate(cursor)} className="w-full rounded-md bg-muted/50 p-3 text-sm text-muted-foreground hover:bg-muted">
    ...
    >
      <div className="mb-1 flex items-center justify-between gap-1">
        ...
        <div className="flex items-center gap-1">
          ...
          {conflictIds.size > 0 && <AlertTriangle className="h-3 w-3 text-destructive" />}
        </div>
      </div>
